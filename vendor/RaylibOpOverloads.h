...
// **************************************
//
// Since Quaternion is a typedef of Vector4 in RayLib, only addition and
// substraction overloads are provided for Vector4, since these work the same
// way with Quaternions as with regular Vectors. To avoid confusion, I did not
// overload Vector4 for negation, multiplication/division or scale, since these
// operations are different for Quaternions vs. Vector4D
...
inline Color operator+(const Color& a, const Color& b) {
    int red, green, blue,
        alpha;  // To ensure that addition of a+b does not result in components
                // greater than unsigned char's max of 255, we upconvert to int,
                // then check the sum, and cast back to unsigned char
    Color c;
    red = (int) a.r + (int) b.r;
...
// Since Quaternion is a Vector4 typedef, I did not provide a negation for
// Vector4 to avoid confusion with QuaternionInvert();
...
inline Color operator-(const Color& a, const Color& b) {
    int red, green, blue, alpha;  // Convert to int, clamp to min zero, convert
...
// Multiplication overload only provides for multiplying a vector by a scalar.
// Vector * Vector is not overloaded to avoid confusion whether one intends a
...
// I'm not sure if the following is useful, but here it is anyway for
// completeness
inline Color operator*(const Color& a, const Color& b) {
    ...
    c.r = (unsigned char) ((
        (red > 255) ? 255
        : (red < 0) ? 0
                    : red));  // Nested ternary clamps return between 0 and 255
...
inline Color operator*(const Color& a, const float b) {
    float red, green, blue,
        alpha;  // We cast to float, clamp, then recast back to unsigned char
    Color c;
    red = (float) a.r * b;
    green = (float) a.g * b;
    blue = (float) a.b * b;
...
// Division overload: Merely scalar multiplication by the reciprocal, with a
...
// I'm not sure if the following is useful either, but here it is for
// completeness
inline Color operator/(const Color& a, const Color& b) {
...
//
// Comparing float values requires care.  Choose EQUALITY_OPERATOR_SIMPLE,
// EQUALITY_OPERATOR_KNUTH, or neither in the #defines at the top of the file
...
// Takes a conservative approach and only affirms that two vectors are equal if
// all of their respective components are equal within machine precision.
#include <cmath>
#include <limits>
inline bool operator==(const Vector2& a, const Vector2& b) {
    bool XIsEqual = false;
    ...
    XIsEqual =
        std::fabs(a.x - b.x) <=
        ((std::fabs(a.x) > std::fabs(b.x) ? std::fabs(b.x) : std::fabs(a.x)) *
         std::numeric_limits<float>::epsilon());
    YIsEqual =
        std::fabs(a.y - b.y) <=
        ((std::fabs(a.y) > std::fabs(b.y) ? std::fabs(b.y) : std::fabs(a.y)) *
         std::numeric_limits<float>::epsilon());
...
inline bool operator==(const Vector3& a, const Vector3& b) {
    ...
    ZIsEqual =
        std::fabs(a.z - b.z) <=
        ((std::fabs(a.z) > std::fabs(b.z) ? std::fabs(b.z) : std::fabs(a.z)) *
         std::numeric_limits<float>::epsilon());
...
inline bool operator==(const Vector4& a, const Vector4& b) {
    ...
    WIsEqual =
        std::fabs(a.w - b.w) <=
        ((std::fabs(a.w) > std::fabs(b.w) ? std::fabs(b.w) : std::fabs(a.w)) *
         std::numeric_limits<float>::epsilon());
...
// Two different formats are provided for printing vectors and colors.  Use the
// #define section at the top of the file to select betweem them. Other
// convenience overloads are provided below which allow for easy printing of
// components of structs.  Some of these make use of the vector format selected
// above when Vectors are part of the struct
...
inline std::ostream& operator<<(std::ostream& os, Color c) {
    os << "R=" << (unsigned int) c.r << " G=" << (unsigned int) c.g
...
inline std::string PixelFormatNumberToName(int format) {
    ...
    switch (format) {
        case (PIXELFORMAT_UNCOMPRESSED_GRAYSCALE): {
            os = "PIXELFORMAT_UNCOMPRESSED_GRAYSCALE, 8 bit per pixel (no "
                 "alpha)";
            break;
        }
        case (PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA): {
            os = "PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA, 8*2 bpp (2 channels)";
            break;
        }
        case (PIXELFORMAT_UNCOMPRESSED_R5G6B5): {
            os = "PIXELFORMAT_UNCOMPRESSED_R5G6B5, 16 bpp";
            break;
        }
        case (PIXELFORMAT_UNCOMPRESSED_R8G8B8): {
            os = "PIXELFORMAT_UNCOMPRESSED_R8G8B8, 24 bpp";
            break;
        }
        case (PIXELFORMAT_UNCOMPRESSED_R5G5B5A1): {
            os = "PIXELFORMAT_UNCOMPRESSED_R5G5B5A1, 16 bpp (1 bit alpha)";
            break;
        }
        case (PIXELFORMAT_UNCOMPRESSED_R4G4B4A4): {
            os = "PIXELFORMAT_UNCOMPRESSED_R4G4B4A4, 16 bpp (4 bit alpha)";
            break;
        }
        case (PIXELFORMAT_UNCOMPRESSED_R8G8B8A8): {
            os = "PIXELFORMAT_UNCOMPRESSED_R8G8B8A8, 32 bpp";
            break;
        }
        case (PIXELFORMAT_UNCOMPRESSED_R32): {
            os = "PIXELFORMAT_UNCOMPRESSED_R32, 32 bpp (1 channel - float)";
            break;
        }
        case (PIXELFORMAT_UNCOMPRESSED_R32G32B32): {
            os = "PIXELFORMAT_UNCOMPRESSED_R32G32B32, 32*3 bpp (3 channels - "
                 "float)";
            break;
        }
        case (PIXELFORMAT_UNCOMPRESSED_R32G32B32A32): {
            os = "PIXELFORMAT_UNCOMPRESSED_R32G32B32A32, 32*4 bpp (4 channels "
                 "- float)";
            break;
        }
        case (PIXELFORMAT_COMPRESSED_DXT1_RGB): {
            os = "PIXELFORMAT_COMPRESSED_DXT1_RGB, 4 bpp (no alpha)";
            break;
        };
        case (PIXELFORMAT_COMPRESSED_DXT1_RGBA): {
            os = "PIXELFORMAT_COMPRESSED_DXT1_RGBA, 4 bpp (1 bit alpha)";
            break;
        }
        case (PIXELFORMAT_COMPRESSED_DXT3_RGBA): {
            os = "PIXELFORMAT_COMPRESSED_DXT3_RGBA, 8 bpp";
            break;
        }
        case (PIXELFORMAT_COMPRESSED_DXT5_RGBA): {
            os = "PIXELFORMAT_COMPRESSED_DXT5_RGBA, 8 bpp";
            break;
        }
        case (PIXELFORMAT_COMPRESSED_ETC1_RGB): {
            os = "PIXELFORMAT_COMPRESSED_ETC1_RGB, 4 bpp";
            break;
        }
        case (PIXELFORMAT_COMPRESSED_ETC2_RGB): {
            os = "PIXELFORMAT_COMPRESSED_ETC2_RGB, 4 bpp";
            break;
        }
        case (PIXELFORMAT_COMPRESSED_ETC2_EAC_RGBA): {
            os = "PIXELFORMAT_COMPRESSED_ETC2_EAC_RGBA, 8 bpp";
            break;
        }
        case (PIXELFORMAT_COMPRESSED_PVRT_RGB): {
            os = "PIXELFORMAT_COMPRESSED_PVRT_RGB, 4 bpp";
            break;
        }
        case (PIXELFORMAT_COMPRESSED_PVRT_RGBA): {
            os = "PIXELFORMAT_COMPRESSED_PVRT_RGBA, 4 bpp";
            break;
        }
        case (PIXELFORMAT_COMPRESSED_ASTC_4x4_RGBA): {
            os = "PIXELFORMAT_COMPRESSED_ASTC_4x4_RGBA, 8 bpp";
            break;
        }
        case (PIXELFORMAT_COMPRESSED_ASTC_8x8_RGBA): {
            os = "PIXELFORMAT_COMPRESSED_ASTC_8x8_RGBA, 2 bpp";
            break;
        }
        default: {
            os = "Unrecognized PixelFormat number";
        }
    }
...
inline std::ostream& operator<<(std::ostream& os, Image i) {
    os << "Image width=" << i.width << " Height=" << i.height
       << " Mipmap levels=" << i.mipmaps << " PixelFormat number:" << i.format
       << " type: " << PixelFormatNumberToName(i.format) << " ";
    return os;
}

inline std::ostream& operator<<(std::ostream& os, Texture t) {
    os << "Texture ID#: " << t.id << " Width=" << t.width
       << " Height=" << t.height << " Mipmap levels=" << t.mipmaps
       << " PixelFormat number:" << t.format
       << " type: " << PixelFormatNumberToName(t.format) << " ";
    return os;
}

inline std::ostream& operator<<(std::ostream& os, Camera2D c) {
    os << "** 2D Camera info. **\nOffset: " << c.offset
       << " Target: " << c.target << " Rotation: " << c.rotation
       << " Zoom=" << c.zoom;
...
// Definition of Camera3D in raylib.h states that fovy is field-of-view aperture
// in perspective mode, but near plane width in orthographic mode
inline std::ostream& operator<<(std::ostream& os, Camera3D c) {
    os << "*** 3D Camera info. ***\nPosition: " << c.position
       << " Target: " << c.target << " Up vector: " << c.up << "\n";
...
#ifdef RAYLIB_OP_OVERLOADS_RAYGUI
inline std::ostream& operator<<(std::ostream& os, const RayHitInfo& rhi) {
    if (rhi.hit) {
        os << "Ray hit. Distance=" << rhi.distance
           << " Position: " << rhi.position
           << " Surface normal: " << rhi.normal;
    } else {
        os << "Ray missed.";
    }
    return os;
...
inline std::ostream& operator<<(std::ostream& os, const CharInfo& ci) {
    os << "Char info:  Char value: " << ci.value << " Offset X: " << ci.offsetX
       << " Offset Y: " << ci.offsetY << " Advance position X: " << ci.advanceX;
    return os;
}

inline std::ostream& operator<<(std::ostream& os, const Font& f) {
    os << "Font info:  Base size (default char height): " << f.baseSize
       << " Number of characters: " << f.charsCount
       << " Padding around chars: " << f.charsPadding;
    return os;
}
#endif
...
inline std::ostream& operator<<(std::ostream& os, const NPatchInfo& np) {
    os << "NPatch info:  Rectangle: " << np.source
       << " Border offsets: Left: " << np.left << " Right: " << np.right
       << " Top: " << np.top << " Bottom: " << np.bottom
       << " Layout: " << np.layout;
    return os;
}
